...
func printRHeader(file *os.File) {
	...
	fmt.Fprintln(file, "practice, targets, speed, oprange, difficulty, addition, target, hits, friendHits, shots, hovers, op1, op2, block, incentive, trial")
}
		...
		for _, trial := range trials {
			...
			if levels != nil {
				...
				for index := 0; index < iterations; index++ {
					fmt.Fprintf(result_file, "%t, %d, %d, %v, %d, %f, %f, %d, %d, %d, %d, %d, %d, %s, %t, %s\n",
						levels.Practice,
